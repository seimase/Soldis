...
public class CustomtextView extends TextView {
    ...
    private int YIGGIVOO_UC = 21;
    private int YIGGIVOO_UC_I = 22;
    private int HELVETICA_BOLD = 23;
    private int HELVETICA_BLACK = 24;
    private int HELVETICA_MEDIUM = 25;
    private int HELVETICA_LIGHT = 26;
    private int HELVETICA_BOOK = 27;
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        }else if (fontName == YIGGIVOO_UC) {
            setFontType(Typeface.createFromAsset(getContext().getAssets(), "Yiggivoo_UC.ttf"));
        }else if (fontName == YIGGIVOO_UC_I) {
            setFontType(Typeface.createFromAsset(getContext().getAssets(), "Yiggivoo_UC_I.ttf"));
        }else if (fontName == HELVETICA_BOLD) {
            setFontType(Typeface.createFromAsset(getContext().getAssets(), "helvetica_bold.otf"));
        }else if (fontName == HELVETICA_BLACK) {
            setFontType(Typeface.createFromAsset(getContext().getAssets(), "helvetica_black.otf"));
        }else if (fontName == HELVETICA_MEDIUM) {
            setFontType(Typeface.createFromAsset(getContext().getAssets(), "helvetica_medium.ttf"));
        }else if (fontName == HELVETICA_LIGHT) {
            setFontType(Typeface.createFromAsset(getContext().getAssets(), "helvetica_light.ttf"));
        }else if (fontName == HELVETICA_BOOK) {
            setFontType(Typeface.createFromAsset(getContext().getAssets(), "helvetica_book.otf"));
        }
