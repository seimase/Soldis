...
import com.soldis.yourthaitea.dashboard.summary_sales.adapter.AdapterDashboardSummarySalesByDepo;
...
public class DashboardSummarySalesByDepo extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
    TextView txtName, txtId, txtLabel, txtLabelList,
            txtTgl,
            txtFab, txtLabelDate,
            txtAchOrg, txtTargetOrg
            ;
    ...
    void InitControl(){
        ...
        txtAchOrg = (TextView)findViewById(R.id.txtAchOrg);
        txtTargetOrg = (TextView)findViewById(R.id.txtTargetOrg);
        ...
        txtLabelList.setText("LIST DEPO");
    ...
    void SyncData(final String sDATE){
        Log.w("DS", sDATE);
        ...
        try{
            Call<Tbl_ListSummarySales_ByDepo> call;
            if(bDays){
                call = NetworkManager.getNetworkService().ListSummarySalesByDepo(
                        AppConstant.strSlsNo,
                        sDATE);
            }else{
                call = NetworkManager.getNetworkService().ListSummarySalesByDepoMTD(
                        AppConstant.strSlsNo,
                        sDATE);
            }
            ...
            call.enqueue(new Callback<Tbl_ListSummarySales_ByDepo>() {
                @Override
                public void onResponse(Call<Tbl_ListSummarySales_ByDepo> call, Response<Tbl_ListSummarySales_ByDepo> response) {
                    progress.dismiss();
                    int code = response.code();
                    if (code == 200){
                        tblListTrxByDepo = response.body();
                        if (!tblListTrxByDepo.error){
                            DATE = sDATE;
                ...
                @Override
                public void onFailure(Call<Tbl_ListSummarySales_ByDepo> call, Throwable t) {
                    progress.dismiss();
                }
            });
        }catch (Exception e){
    ...
    void FillGrid(){
        ...
        long lTarget = 0, lAch = 0;
        for (Tbl_ListSummarySales_ByDepo.DataOrg dat : tblListTrxByDepo.data_org){
            lTarget += dat.TARGET_EC;
            lAch += dat.TOTAL_EC;
        }

        txtTargetOrg.setText(AppController.getInstance().toCurrency(lTarget));
        txtAchOrg.setText(AppController.getInstance().toCurrency(lAch));

        if (lTarget > 0){
            double lPercen = ((double)lAch/lTarget) *100;
            if (lPercen > 100){
                pieView.setPercentage(100);
            }else{

                pieView.setPercentage((long)lPercen);
            }
        }else  {
            pieView.setPercentage(0);
        }

        mAdapter = new AdapterDashboardSummarySalesByDepo(this, tblListTrxByDepo, bDays, DATE, new AdapterDashboardSummarySalesByDepo.OnDownloadClicked() {
            @Override
    ...
    void showDialodDatePick(){
        ...
        DatePickerDialog dpd = DatePickerDialog.newInstance(
                DashboardSummarySalesByDepo.this,
