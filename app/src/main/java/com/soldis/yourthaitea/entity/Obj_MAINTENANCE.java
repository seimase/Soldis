...
public class Obj_MAINTENANCE {
    ...
    public Obj_MAINTENANCE getData(String CUSTNO){
        try{
            Cursor cur = AppConstant.myDb.rawQuery("SELECT * FROM TBL_MAINTENANCE WHERE CUSTNO = '"+CUSTNO+"'", null);
            if (cur.getCount() > 0){
                cur.moveToFirst();
    ...
    public ArrayList<Obj_MAINTENANCE> getDataList() {
        ArrayList<Obj_MAINTENANCE> AlisData = new ArrayList<Obj_MAINTENANCE>();

        try{
            String sSql = "SELECT * FROM TBL_MAINTENANCE";
            Cursor cur = AppConstant.myDb.rawQuery(sSql, null);

            if (cur.getCount() > 0){
//			c.moveToFirst();
                while (cur.moveToNext()){
                    Obj_MAINTENANCE prodDat = new Obj_MAINTENANCE();
    ...
    public void CreateData(Obj_MAINTENANCE dat){
        try{
            String sSql = "INSERT INTO TBL_MAINTENANCE(" +
                    "CUSTNO," +
                    "Q1," +
                    "Q2," +
                    "Q3," +
            ...
            Log.w("Insert TBL_MAINTENANCE", e.getMessage());
        }
    }

    public void UpdateData(Obj_MAINTENANCE dat){
        try{
            String sSql = "UPDATE TBL_MAINTENANCE" +
                    " SET" +
                    " Q1 = '"+dat.getQ1()+"'," +
                    " Q2 = '"+dat.getQ2()+"'," +
                    " Q3 = '"+dat.getQ3()+"'," +
            ...
            Log.w("Update TBL_MAINTENANCE", e.getMessage());
        }
    }
    
    public void DeleteData(){
        try{
            String sSql = "DELETE FROM TBL_MAINTENANCE" ;
